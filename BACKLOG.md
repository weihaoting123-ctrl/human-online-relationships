...
需求池修订：R1 · 2026-09-06。审查基线 `94180d0` / 软件版本 `0.1.0`。R1 是需求文档修订，不是软件 Release。
...
- 当前新增项全部是候选。已有 #7–#10 保持开放及原里程碑；以下版本建议未自动移动它们。
...
建议批准 A 批作为 v0.1.1：只修复导入、结果导航、空状态、焦点与错误提示。B–D 是完整需求池，不意味着可以跳过其设计、权限和平台验收。

批准后，按独立实现规范与测试计划开发；未批准前不把本表移为“实现中”。完成代码和验证后才创建新版本标签/Release，并更新此处的实际版本记录。

## GitHub 主待办映射

已核对既有待办，新增 #12–#19；各项仅为开放候选，不自动分配新里程碑或关闭旧 Issue。未列独立 Issue 的需求保留在池中，进入详细设计时再拆分；涉及潜在安全问题的细节保持非公开。
...
| 本次解构与需求池 R1 | 审查草案已整理；实现范围待确认 | 本文件与 [解构报告](docs/project-decomposition.md) |
| v0.1.1 及后续候选 | 未实现、未发布 | 不以需求单、草案 PR 或旧 CI 代替完成证据 |
