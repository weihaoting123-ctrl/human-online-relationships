...
- Windows 对话副驾候选已加入：[安装、无感窗口行为与隐私边界](docs/copilot.md)。回复助手页面顶部可一键启动或打开悬浮窗口，无需日常执行命令。支持吸附/悬浮、一键标题校准、按次回复建议，以及另行授权的固定私聊新文字辅助（最多 15 分钟 / 6 次尝试）。默认不读取新消息或调用模型，标题不代表当前账号/联系人身份；不自动换人、不代发、实际读取有延迟。`1.0.0-rc.5` 不代表正式 1.0 发布，真实来源与模型效果仍须单独验收。
